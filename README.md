...
4. On a new tab, go to `Settings` > `Security` > `Secrets` > `Actions`
5. Click `New repository secret`, then type `CLIENT_ID` in the Name field. Paste
   the copied string from step 4 into the value field, then click `Add secret`.
6. Back in the developer console, click `New secret`, then copy the resulting value.
7. On GitHub, click `New repository secret`, then type `CLIENT_SECRET` into the Name field. Paste the copied value from step 7 into the value field, then click `Add secret`.
...
11. Done! Your webhook will be triggered when the channel goes live, or goes offline.
